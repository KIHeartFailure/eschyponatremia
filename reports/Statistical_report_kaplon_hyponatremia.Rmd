...
Variables included in the imputation model are indicated in Table \ref{tab:tab1all}. 
The primary outcome, all-cause death or first re-hospitalization for HF at follow-up, was included as the Nelson-Aalen estimator. 
Natremia was not included in the model
...
The outcome analysis is performed on the long-term outcome population (patients with follow-up data).
...
used to model the time to first event. Adjustment was performed for variables indicated in Table \ref{tab:tab1all} 
and the selection was based on clinical judgement. 
...
residuals (cox.zph in [@survival-package]). => XXX
...
Modelled using logistic regression including the variables shown in the table. 
Performed on the long-term outcome population (patients with follow-up data)???
